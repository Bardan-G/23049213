    ...
    <Link href={`/shop/${product.id}`} className="group">
      <div className="border rounded-lg p-4 shadow-sm hover:shadow-xl transition-all duration-300 bg-white h-full flex flex-col">
        ...
        <div className="flex justify-between items-center mt-auto">
          ...
          {/* Note: Clicking this button will currently also trigger the Link. 
              We will fix the "Add to Cart" propagation when we build the cart logic. */}
          <button 
            className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors shadow-sm"
            onClick={(e) => {
              e.preventDefault(); // Prevents navigating to Detail Page when clicking 'Add'
              alert('Added to cart!'); // Placeholder for Zustand logic
            }}
          >
            Add to Cart
          </button>
